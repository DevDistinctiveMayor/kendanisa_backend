 ...
 *   description: Handle visa and documentation-related service requests
 */
 ...
 *     summary: Submit a visa or documentation assistance request
 *     description: Send a visa/documentation request that will be emailed to admin and confirmed to the client.
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 ...
 *               serviceType:
 *                 type: string
 *                 enum: [Visa Application, Passport Renewal, Residency Permit, Travel Documentation, Other]
 ...
 *               message:
 *                 type: string
 *                 example: "I need assistance with UK tourist visa."
 *     responses:
 *       200:
 *         description: Visa request sent successfully
 *         content:
 *           application/json:
 *             example:
 *               success: true
...
router.post('/visa-request', visaRequestLimiter, async (req, res) => {
  try {
    const { fullName, email, phone, serviceType, destinationCountry, preferredContactMethod, message } = req.body;
          ...
          <div class="container">
            ...
            <div class="content">
              ...
              <div class="info-card">
                ...
                ${preferredContactMethod ? `<div class="info-row"><div class="label">Preferred Contact:</div><div class="value">${preferredContactMethod}</div></div>` : ''}
                <div class="info-row"><div class="label">Service Type:</div><div class="value">${serviceType}</div></div>
                ${destinationCountry ? `<div class="info-row"><div class="label">Destination:</div><div class="value">${destinationCountry}</div></div>` : ''}
              ...
              ${message ? `<div class="info-card"><div class="label">Message:</div><div class="value">${message}</div></div>` : ''}
